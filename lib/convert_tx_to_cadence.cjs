...
const files = [
  ...
  __dirname + "/scripts/DapperGetPrimaryListingMetadata.cdc",
  __dirname + "/scripts/DapperGetSecondaryListingMetadata.cdc"
]
...
for (let fileName in files ) {
  ...
  cadenceCode.push(`pub fun ${funcName}(nftSchema: TransactionGenerationUtils.NFTSchema?, ftSchema: TransactionGenerationUtils.FTSchemaV2?): String {`)
