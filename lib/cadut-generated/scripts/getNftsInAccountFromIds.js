...
/**
* Method to generate cadence code for getNftsInAccountFromIds script
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const getNftsInAccountFromIdsTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `getNftsInAccountFromIds =>`)

  return replaceImportAddresses(CODE, fullMap);
};

export const getNftsInAccountFromIds = async (props = {}) => {
  const { addressMap = {}, args = [] } = props
  const code = await getNftsInAccountFromIdsTemplate(addressMap);

  reportMissing("arguments", args.length, 2, `getNftsInAccountFromIds =>`);
