...
/**
* Method to generate cadence code for getNftsInAccountFromPath script
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const getNftsInAccountFromPathTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `getNftsInAccountFromPath =>`)

  return replaceImportAddresses(CODE, fullMap);
};

export const getNftsInAccountFromPath = async (props = {}) => {
  const { addressMap = {}, args = [] } = props
  const code = await getNftsInAccountFromPathTemplate(addressMap);

  reportMissing("arguments", args.length, 2, `getNftsInAccountFromPath =>`);
