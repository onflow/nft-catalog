...
/**
* Method to generate cadence code for getNftInAccount script
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const getNftInAccountTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `getNftInAccount =>`)

  return replaceImportAddresses(CODE, fullMap);
};

export const getNftInAccount = async (props = {}) => {
  const { addressMap = {}, args = [] } = props
  const code = await getNftInAccountTemplate(addressMap);

  reportMissing("arguments", args.length, 3, `getNftInAccount =>`);
