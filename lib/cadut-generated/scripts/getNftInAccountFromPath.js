...
/**
* Method to generate cadence code for getNftInAccountFromPath script
* @param {Object.<string, string>} addressMap - contract name as a key and address where it's deployed as value
*/
export const getNftInAccountFromPathTemplate = async (addressMap = {}) => {
  ...
  reportMissingImports(CODE, fullMap, `getNftInAccountFromPath =>`)

  return replaceImportAddresses(CODE, fullMap);
};

export const getNftInAccountFromPath = async (props = {}) => {
  const { addressMap = {}, args = [] } = props
  const code = await getNftInAccountFromPathTemplate(addressMap);

  reportMissing("arguments", args.length, 3, `getNftInAccountFromPath =>`);
